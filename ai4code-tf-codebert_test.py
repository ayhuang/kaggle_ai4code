...
input_ids, attention_mask, segment_ids, labels = tokenize_and_label(test_df, dict(zip(test_df['cell_id'].values, test_df['source'].values)), mode='test')
...
print( f'total no. of test cells: {len( test_df)}, total no. of pairwise test examples: {len(y_pred)}')
preds_copy = y_pred
